...
export default function RoommatePreferencesPage() {
  ...
  const handleSubmit = async (data: { profile: Partial<RoommateProfile>; preferences: Partial<RoommatePreference> }) => {
    try {
      const { createOrUpdateRoommateProfile, createOrUpdateRoommatePreferences } = await import('@/services/matching-service');
      const { createClient } = await import('@/lib/supabase/client');
      const supabase = createClient();
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;

      await createOrUpdateRoommateProfile({ ...data.profile, student_id: user.id });
      await createOrUpdateRoommatePreferences({ ...data.preferences, student_id: user.id });
      setSaved(true);
    } catch (error) {
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center mb-8">
            ...
            <p className="text-neutral-600">Tell us about yourself and what you&apos;re looking for.</p>
          </div>
