...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'approve') {
    const result = await OpportunitySuggestionService.approveSuggestion(id, user.id)
    if (result.error) {
  ...
  if (action === 'reject') {
    ...
    const result = await OpportunitySuggestionService.rejectSuggestion(id, user.id, rejectionReason)
    if (result.error) {
      return NextResponse.json({ error: result.error }, { status: 500 })
    }
    return NextResponse.json({ success: true })
