      ...
      <section className="relative overflow-hidden bg-white pt-16 pb-32">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
          <div className="lg:grid lg:grid-cols-2 lg:gap-16 items-center">
            ...
            >
              <Badge variant="success" className="mb-6 px-4 py-1.5 text-sm uppercase tracking-wider font-bold">
                Live at 50+ Campuses
              </Badge>
              <h1 className="text-5xl lg:text-7xl font-extrabold text-neutral-900 tracking-tight leading-[1.1] mb-8">
                Your Student <span className="text-primary italic">Trust</span> Network.
              </h1>
              <p className="text-xl text-neutral-600 leading-relaxed mb-10 max-w-lg font-medium">
                Stop guessing where to live. Join thousands of students across Africa building a verified housing ecosystem based on intelligence, not myths.
              </p>
              
              <div className="flex flex-col sm:flex-row gap-4 mb-12">
                <Link href="/discovery" className="flex-1 sm:flex-none">
                  <Button size="lg" className="w-full sm:w-auto gap-2 text-lg font-bold shadow-xl shadow-primary/20">
                    Find Housing <ArrowRight className="h-5 w-5" />
                  </Button>
                </Link>
                <Link href="/signup">
                  <Button variant="outline" size="lg" className="w-full sm:w-auto text-lg font-bold border-2">
                    Join the Network
                  </Button>
                </Link>
              </div>
              ...
              <div className="flex items-center gap-6">
                <div className="flex -space-x-3">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="h-12 w-12 rounded-full border-4 border-white bg-neutral-200 overflow-hidden shadow-sm">
                      <Image 
                ...
                <div className="text-sm">
                  ...
                  <p className="text-neutral-500 font-bold uppercase tracking-tight">Trusted by African Students</p>
                </div>
              </div>
            </motion.div>
              ...
              <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border-[12px] border-white ring-1 ring-neutral-200">
                ...
                <div className="absolute bottom-8 left-8 p-6 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl max-w-xs border border-white/50">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="font-black text-neutral-900 uppercase tracking-tighter text-sm italic">Verified Stay</span>
                  </div>
                  <p className="text-xs text-neutral-600 leading-relaxed font-bold">
                    "Finally found a place with reliable water and wifi. The Campozy Score didn't lie!"
                  </p>
                </div>
              </div>
            </motion.div>
      ...
      <section className="py-32 bg-primary relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6">
             <Link href="/signup">
                <Button size="lg" className="bg-white text-primary hover:bg-neutral-100 px-12 h-20 text-2xl font-black rounded-2xl shadow-2xl">
                  Get Started Free
                </Button>
             </Link>
          </div>
...
function FeatureCard({ icon, title, desc, delay, active }: { icon: React.ReactNode, title: string, desc: string, delay: number, active?: boolean }) {
  return (
    <motion.div 
      className={cn(
        "p-12 rounded-[2.5rem] border transition-all",
        active ? "bg-white shadow-2xl border-primary/10 scale-105 z-10" : "bg-white/50 border-neutral-200 hover:bg-white hover:shadow-xl"
      )}
    ...
    >
      <div className={cn(
        ...
        {React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement, { className: 'h-8 w-8' }) : icon}
      </div>
      <h3 className="text-2xl font-black mb-4 text-center tracking-tight uppercase italic">{title}</h3>
      <p className="text-neutral-500 leading-relaxed text-center font-medium">{desc}</p>
    </motion.div>
  )
}
