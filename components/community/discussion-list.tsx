...
export default function DiscussionList({ discussions, className = '', ctaPath, userId }: Props) {
  ...
  const handleDesktopClick = React.useCallback((id: string) => {
        ...
        <div className="space-y-4">
          {discussionsState.map((discussion) => (
            <DiscussionCard
              ...
              className={expandedId === discussion.id ? 'transition-colors' : 'hover:border-blue-400 hover:bg-blue-50 hover:shadow-blue-200/60 transition-colors'}
            />
          ))}
        </div>
