...
export function PropertyCard({ property, index = 0 }: PropertyCardProps) {
  ...
  const placeholder = 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?auto=format&fit=crop&q=80&w=800'
        ...
        <div className="overflow-hidden rounded-2xl bg-white border border-neutral-200 transition-all hover:shadow-2xl hover:border-primary/20">
          ...
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            <div className="absolute top-3 left-3 flex gap-2">
              <Badge variant="secondary" className="backdrop-blur-md bg-white/90 text-primary border-none shadow-sm">
                {property.property_type?.name || 'Hostel'}
              </Badge>
              {property.verification_level !== 'unverified' && (
                <Badge variant="success" className="backdrop-blur-md bg-white/90 border-none shadow-sm">
                  Verified
                </Badge>
              )}
            </div>
            <div className="absolute top-3 right-3">
               <CampozyScore score={property.campozy_score} size="sm" showLabel={false} className="bg-white/90 backdrop-blur-md rounded-full shadow-sm" />
            </div>
          </div>
          ...
          <div className="p-5">
            ...
            <div className="flex items-center gap-1.5 text-neutral-400 text-sm mb-4">
              ...
              <span className="line-clamp-1">{property.neighborhood?.name}</span>
            </div>

            <div className="flex items-center gap-4 mb-6 pt-4 border-t border-neutral-50">
              <UtilityIcon icon={<Droplet />} active={property.utilities?.some(u => u.utility_type?.name === 'Water')} />
              <UtilityIcon icon={<Zap />} active={property.utilities?.some(u => u.utility_type?.name === 'Electricity')} />
              <UtilityIcon icon={<Wifi />} active={property.utilities?.some(u => u.utility_type?.name === 'Internet')} />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="text-neutral-400 text-[10px] uppercase font-bold ml-1 tracking-widest">KES / mon</span>
              </div>
...
function UtilityIcon({ icon, active }: { icon: React.ReactNode, active?: boolean }) {
  return (
    <div className={cn(
